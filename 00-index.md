---
background: https://cover.sli.dev
title: Workshop Name - Slide Decks
info: |
  ## Workshop Slide Decks
  Collection of lesson slide decks for workshops.
class: text-center
drawings:
  persist: false
transition: slide-left
mdc: true
seoMeta:
  ogImage: auto
# Build configuration
base: /00-index/
out: ./dist/00-index
theme: '@workshops.de/slidev-theme'
---

# Workshop Name

Slide Deck Collection

<div class="mt-12">

## Available Lessons

</div>

<div class="grid grid-cols-1 gap-4 mt-8 text-left max-w-xl mx-auto">

<a href="../01-example-lesson/" class="p-4 rounded-lg bg-white bg-opacity-10 hover:bg-opacity-20 transition-all no-underline">
  <h3 class="text-xl font-bold">01 - Example Lesson</h3>
  <p class="text-sm opacity-70">This is an example lesson to demonstrate the structure</p>
</a>

<a href="../02-architecture/" class="p-4 rounded-lg bg-white bg-opacity-10 hover:bg-opacity-20 transition-all no-underline">
  <h3 class="text-xl font-bold">02 - Architecture</h3>
  <p class="text-sm opacity-70">Important Principles, Possibilities & Techniques</p>
</a>

<!-- Add more lessons here as you create them -->

</div>

<div class="abs-br m-6 text-xl">
  <a href="https://github.com/workshops-de/workshop-slides-template" target="_blank" class="slidev-icon-btn">
    <carbon:logo-github />
  </a>
</div>
